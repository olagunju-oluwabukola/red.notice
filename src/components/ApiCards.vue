<template>
  <section class="hidden md:block p-6 bg-gradient-to-r from-[#160101] to-[#151212] text-white">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 hidden md:grid">
      <div
        v-for="(card, index) in monitoringCards"
        :key="index"
        class="p-4 rounded-lg card"
      >
        <div class="relative bg-black rounded-lg overflow-hidden">
          <img :src="card.image" alt="API Monitoring" class="object-cover" />
        </div>
        <div class="mt-4">
          <h3 class="text-red text-[10px] font-bold">PROFILE PROTECTION</h3>
          <h4 class="text-[10px] font-semibold mt-2">{{ card.title }}</h4>
          <p class="mt-2 text-[8px] text-gray-400">{{ card.description }}</p>
        </div>
      ...
      <!-- "Set up new API Monitoring" button -->
      <div
        class="flex flex-col items-center justify-center p-4 rounded-lg shadow-md card cursor-pointer"
        @click="showForm = true"
      >
    ...
    <div class="md:hidden flex flex-col items-center">
      <div class="relative w-[80%] mx-auto overflow-hidden">
        <!-- Slider Container -->
        <div
